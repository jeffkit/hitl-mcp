...
## 0. 架构演进（2026-06-29）：内置引擎单进程

本文档最初设计的是"独立 Worker 进程 + WS 注册"的共享轮询后端。**已落地后随即演进为"内置引擎单进程"**，部署更轻。原设计内容（§1–§10）保留作为背景与远程部署方案的参考。
...
- **WS 外部 Worker 接口保留**：`ws_manager` 多上游路由不删。`/api/send` 与 `/api/ilink/*` 逻辑为"内置引擎优先，回退 WS"。现有 fly-pigeon / devcloud-worker / 远端 pro 部署不受影响。
- **`packages/ilink-worker` 保留**：降级为"远程部署可选方案"（凭证不在本机、长轮询需在另一台机器跑时使用）。默认本机部署不用它，改用内置引擎。
...
未来按同样模式新增 `hil_server/engines/wecom_aibot.py`，配置加 `ENABLE_WECOM_AIBOT_ENGINE`，无需独立 worker 包。
